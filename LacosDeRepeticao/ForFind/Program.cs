...
namespace ForFind
{
    class Program
    {
        ...
        private static int FindByWhile(string wordToFind)
            {
                var contentToFind = TextContent();
                var wordFindCountLength = wordToFind.Length;
                var lenghtText = contentToFind.Length - (wordFindCountLength - 1);
                var countWordFind = 0;

                for (int i = 0; i < lenghtText; i++)
                {
                    var compareWord = string.Empty;
                for (int b = 0; b < wordFindCountLength; b++)
